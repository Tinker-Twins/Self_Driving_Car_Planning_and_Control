...
PLOT_LEFT          = 0.05   # in fractions of figure width and height
PLOT_BOT           = 0.05
PLOT_WIDTH         = 0.9
PLOT_HEIGHT        = 0.9
...
# Motion planning and control pecific parameters
C4_STOP_SIGN_FILE        = 'Stop_Sign_Parameters.txt'
C4_STOP_SIGN_FENCELENGTH = 5        # m
C4_PARKED_CAR_FILE       = 'Parked_Vehicle_Parameters.txt'
...
def exec_waypoint_nav_demo(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        # simulator starting time. Note that we also need to
        ...
        lp_traj = lv.LivePlotter(tk_title="Trajectory Trace", win_pos=[1450,125])
        ...
        trajectory_fig.set_invert_y_axis() # Invert Y-axis
        trajectory_fig.set_axis_equal()    # X-Y spacing should be equal in size

        # Add waypoint markers
        trajectory_fig.add_graph("waypoints", window_size=waypoints_np.shape[0],
                                 x0=waypoints_np[:,1], y0=waypoints_np[:,0],
                                 linestyle="-", marker="", color='gray')
        # Add trajectory markers
        trajectory_fig.add_graph("trajectory", window_size=TOTAL_EPISODE_FRAMES,
                                 x0=[start_y]*TOTAL_EPISODE_FRAMES,
                                 y0=[start_x]*TOTAL_EPISODE_FRAMES,
                                 color='deepskyblue')
        # Add starting position marker
        trajectory_fig.add_graph("start_pos", window_size=1,
                                 x0=[start_y], y0=[start_x],
                                 marker='D', color='red', markertext="Start",
                                 marker_text_offset_x=0, marker_text_offset_y=7.5)
        # Add end position marker
        trajectory_fig.add_graph("end_pos", window_size=1,
                                 x0=[waypoints_np[-1, 1]],
                                 y0=[waypoints_np[-1, 0]],
                                 marker="D", color='red', markertext="  End",
                                 marker_text_offset_x=0, marker_text_offset_y=2)
        # Add car marker
        trajectory_fig.add_graph("car", window_size=1,
                                 marker="s", color='blue', markertext="Car",
                                 marker_text_offset_x=0, marker_text_offset_y=0)
        # Add lead car information
        trajectory_fig.add_graph("leadcar", window_size=1,
                                 marker="s", color='magenta', markertext="Lead Car",
                                 marker_text_offset_x=0, marker_text_offset_y=0)
        # Add stop sign position
        trajectory_fig.add_graph("stopsign", window_size=1,
                                 x0=[stopsign_fences[0][1]], y0=[stopsign_fences[0][0]],
                                 marker="H", color="red", markertext="Stop Sign",
                                 marker_text_offset_x=0, marker_text_offset_y=7)
        # Add stop sign "stop line"
        trajectory_fig.add_graph("stopsign_fence", window_size=1,
                                 x0=[stopsign_fences[0][1], stopsign_fences[0][3]],
                                 y0=[stopsign_fences[0][0], stopsign_fences[0][2]],
                                 color="red")
        ...
        trajectory_fig.add_graph("parkedcar_pts", window_size=parkedcar_box_pts_np.shape[0],
                                 x0=parkedcar_box_pts_np[:,1], y0=parkedcar_box_pts_np[:,0],
                                 linestyle="", marker="+", color='magenta')

        # Add lookahead path
        trajectory_fig.add_graph("selected_path",
                                 window_size=INTERP_MAX_POINTS_PLOT,
                                 x0=[start_y]*INTERP_MAX_POINTS_PLOT,
                                 y0=[start_x]*INTERP_MAX_POINTS_PLOT,
                                 color='green',
                                 linewidth=3)
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            if frame % LP_FREQUENCY_DIVISOR == 0:
                # Once you have completed the prerequisite functions of each of these
                ...
                print("Current Vehicle Velocity = %f m/s" % current_speed)
                ...
                print("Optimal Path Index = %d" % best_index)
                ...
                print("Desired Vehicle Velocity = %f m/s" % desired_speed)
            ...
            if skip_first_frame and frame == 0:
                ...
                trajectory_fig.roll("trajectory", current_y, current_x)
                trajectory_fig.roll("car", current_y, current_x)
                if lead_car_pos:    # If there exists a lead car, plot it
                    trajectory_fig.roll("leadcar", lead_car_pos[1][1],
                                        lead_car_pos[1][0])
                    ...
                    for i in range(NUM_PATHS):
                        ...
                        if path_validity[i]:
                            ...
                            if not collision_check_array[path_counter]:
                                ...
                                colour = 'gray'
                            trajectory_fig.update("local_path" + str(i), paths[path_counter][1], paths[path_counter][0], colour)
                            path_counter += 1
                        else:
                            trajectory_fig.update("local_path" + str(i), [ego_state[1]], [ego_state[0]], 'red')
